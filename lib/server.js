...
*/
// auth socket
  var jwtSecret="Your secret";
  var sio = io.listen(this,{'log level': 0});
...
Server.prototype.handleRequest = function handleRequest (req, res) {
  ...
  setupReqRes(req, res, function(err, next) {
    ...
    server.sessions.createSession(req.headers.authorization, function(err, session) {
      ...
      if(err) {
       ...
       // how to send sid to the client without set-cookie?
        req.session = session;
