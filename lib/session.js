  ...
  if(sockets) {
    sockets.on('connection', function (socket) {
     if(socket.handshake.decoded_token){
       var  sid=socket.handshake.decoded_token.id;
        ...
        if (/^Bearer$/i.test(scheme)) {
          token = credentials;
           jwt.verify(token,secret,function(err,data){
             ...
             var sess = sessionIndex[data.id] || new Session(data, store, socketIndex, store.sockets);
             sessionIndex[data.id] = sess;
             if(data && data.uid){
              userSessionIndex[data.uid] = sess; 
             }
              fn(err, sess);
          })
        }else{
